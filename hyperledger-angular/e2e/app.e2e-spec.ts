...
describe('Starting tests for hyperledger-angular', function() {
  ...
  it('website title should be hyperledger-angular', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('hyperledger-angular');
    })
  });
  ...
  it('navbar-brand should be hyperledger-angular',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('hyperledger-angular');
    });
  });
